...
import sparksFrag from "./shaders/firework.frag";
import sparksVert from "./shaders/firework.vert";

import tracksFrag from "./shaders/tracks.frag";
import tracksVert from "./shaders/tracks.vert";

export class FireworkMode {
  ...
  generateParticlesData(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const ttl = 50;
      // const radius = 0.3 + Math.random() * 0.001;
      const radius = 0.3;
  ...
  generateParticles(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const x = (dx * 1000) % xMax;
      const y = (dy * 1000) % yMax;
      particles.push(new FireworkParticle([x, y, 0], [dx, dy, 0], ttl));
    }
