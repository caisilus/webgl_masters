...
import steamFrag from "./shaders/fontain.frag";
import steamVert from "./shaders/fontain.vert";

import WaterImage from "./images/water.png";
import { SteamParticle } from "./steam_particle";
import { FontainParticle } from "./fontain_particle";

export class FontainMode implements ParticlesMode {
  gl: WebGL2RenderingContext;
  sparksProgram: ShaderProgram;
  particles: FontainParticle[];
  camera: Camera;
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.particles = this.generateParticles(300);
  ...
  generateParticlesData(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const dx = xMax / ttl;
      const dy = Math.abs(yMax / ttl) + 0.02;
  ...
  generateParticles(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const x = (dx * 1000) % xMax;
      const y = (dy * 1000) % yMax;
      particles.push(new FontainParticle([x, y, 0], [dx, dy, 0], ttl));
    }
